...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'anigadgets_curator') THEN
    CREATE ROLE anigadgets_curator NOLOGIN;
  END IF;
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'anigadgets_importer') THEN
    CREATE ROLE anigadgets_importer NOLOGIN;
  END IF;
...
-- Legacy roles cannot read private tables or manufacture matches/rewrite history.
...
GRANT SELECT ON public.catalogue_public, public.catalogue_offers_public,
  public.catalogue_history_public, public.catalogue_evidence_public TO anigadgets_reader;
GRANT USAGE ON SCHEMA public TO anigadgets_curator, anigadgets_importer;
REVOKE ALL ON public.catalogue_products, public.merchants, public.offers,
  public.offer_observations, public.evidence_records FROM anigadgets_curator, anigadgets_importer;
GRANT SELECT, INSERT, UPDATE ON public.catalogue_products, public.merchants,
  public.offers, public.evidence_records TO anigadgets_curator;
GRANT SELECT (id, curation_key) ON public.offers TO anigadgets_importer;
GRANT SELECT, INSERT ON public.offer_observations TO anigadgets_curator, anigadgets_importer;
GRANT USAGE ON SEQUENCE public.catalogue_products_id_seq, public.merchants_id_seq,
  public.offers_id_seq, public.evidence_records_id_seq TO anigadgets_curator;
GRANT USAGE ON SEQUENCE public.offer_observations_id_seq TO anigadgets_curator, anigadgets_importer;
COMMIT;